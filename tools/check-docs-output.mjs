...
if (!(await exists(distDir))) {
  console.error("Expected built docs output at docs/.vitepress/dist.")
  process.exit(1)
}

const missing = []
for (const rel of requiredFiles) {
  if (!(await exists(path.join(distDir, rel)))) {
    missing.push(`docs/.vitepress/dist/${rel}`)
  }
}
...
if (htmlPages < 3) {
  missing.push(`expected multiple built html pages, found ${htmlPages}`)
}
