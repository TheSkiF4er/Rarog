...
function hasAnyScript(pkg, names) {
  return names.some((name) => pkg.scripts?.[name]);
}

function workflowContains(rel, snippets) {
  const text = readText(rel);
  return snippets.every((snippet) => text.includes(snippet));
}

function workflowContainsAny(rel, snippetGroups) {
  const text = readText(rel);
...
const checks = [
  ...
  ["package.json", () => rootPkg.scripts?.["docs:lint"], "root docs:lint script exists"],
  ...
  [".github/workflows/ci.yml", () => workflowContainsAny(".github/workflows/ci.yml", [["npm run docs:lint"], ["npm run docs:check"]]), "ci workflow runs a docs gate"],
  [".github/workflows/ci.yml", () => workflowContains(".github/workflows/ci.yml", ["npm run build:all"]), "ci workflow builds all packages"],
  [".github/workflows/docs.yml", () => workflowContains(".github/workflows/docs.yml", ["npm run docs:check"]), "docs workflow runs full docs:check"],
  [".github/workflows/release.yml", () => workflowContainsAny(".github/workflows/release.yml", [["npm run test:release"], ["npm run test:unit", "npm run test:adapters", "npm run test:contracts"]]), "release workflow runs a release test gate"],
  [".github/workflows/release.yml", () => workflowContains(".github/workflows/release.yml", ["npm run release:verify", "npm run build:all", "npm run pack:packages"]), "release workflow verifies, builds, and packs"],
  [".github/workflows", () => !hasFile("ci.yml.tmp"), "no temporary ci.yml.tmp file exists in repo root"]
];
