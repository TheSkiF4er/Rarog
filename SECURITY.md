...
**Рарог — Политика безопасности**

Этот документ описывает, как сообщать об уязвимостях, как мы их обрабатываем и какие меры предпринимаются для защиты пользователей и пользователей Рарог. Проект — с открытым исходным кодом под лицензией Apache‑2.0. Автор: TheSkiF4er.
...
* **Альтернативно:** письмо на адрес **[security@cajeer.ru](mailto:___KEEP___1___)**. При отправке письма укажите в теме: `SECURITY REPORT — [краткое описание]`.
...
3. **Разработка патча:** для подтверждённых проблем мы создаём приватную ветку/PR с исправлением. Если исправление влияет на интерфейс, мы готовим инструкцию по миграции.
...
* Рарог — open source проект, текущая политика: **непредусмотрение денежной награды** по умолчанию. Мы всегда готовы публично признать вклад исследователей (если они согласны).
...
* Рекомендуем использовать зафиксированные версии и публиковать инструкции по обновлению зависимостей в выпуск notes.
...
Если вы предполагаете, что секрет (ключ интерфейс, токен доступа) был скомпрометирован в результате уязвимости — сообщите немедленно. Мы можем инициировать срочную ротацию ключей и выпустить инструкции по безопасной миграции.
...
* Email для безопасности: **[security@cajeer.ru](mailto:___KEEP___1___)**.
...
## Best practices для пользователей Рарог
...
Спасибо за помощь в улучшении безопасности Рарог. Мы ценим вклад сообщества и стремимся к прозрачной, быстрой и честной обработке уязвимостей.
