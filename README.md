...
В мире CSS есть две ветки:

1. Утилитарные фреймворки (Tailwind)
2. Компонентные фреймворки (Bootstrap, Material)

**Rarog создаёт третью ветку — ЭЛЕМЕНТНО-ОРИЕНТИРОВАННЫЙ ПОДХОД.**
...
### **Что делает Rarog уникальным? (и нигде не повторённым)**

## **1. Элементно-ориентированная архитектура**
Вместо «утилит» и «компонентов» в Rarog есть **Elements** — сущности, описывающие состояние, форму и поведение UI-элемента.
Например:
* `el-flow` — элементы потоков
* `el-core` — базовые конструкты
* `el-knit` — элементы связывания
* `el-skin` — отвечающие за визуальные оболочки
* `el-state-*` — состояние объекта
* `el-react-*` — реакции взаимодействия
Это больше похоже на **систему частиц**, чем на Tailwind’овские классы.

## **2. Полностью оригинальные имена классов**
Никаких `flex`, `p-4`, `btn` и тысяч других штампов.
В Rarog используется **семиотическая система имён**, вдохновлённая:
* моделированием потоков
* формообразованием
* поведением природных элементов
* терминами из физики и математики
* образами из славянской мифологии
Примеры:
```
shape-round-md     — Формообразование
flux-row           — Потоковое расположение
pulse-fast         — Анимации импульса
skin-glow          — Светящаяся визуальная оболочка
react-press        — Реакция на нажатие
bind-gap-xl        — Связующее пространство
core-align-mid     — Базовое выравнивание
```
Это НЕ встречается ни в одном существующем фреймворке.

## **3. Двухуровневая система токенов**
Rarog вводит **Dual Token Grid**:
**1) Force Tokens** — выражают фундаментальные силы интерфейса:
* поток (`flow-*`)
* плотность (`dens-*`)
* импульс (`imp-*`)
* масса (`mass-*`)
* напряжение (`tens-*`)
**2) Skin Tokens** — визуальная оболочка:
* глубина (`depth-*`)
* оттенок (`tone-*`)
* свечение (`glow-*`)
* текстура (`tex-*`)
* мягкость (`soft-*`)
Это позволяет создавать по-настоящему уникальные UI-языки.

## **4. Rarog не использует традиционный “spacing scale”**
Вместо этого он использует систему **плотности интерфейса**.
Пример:
```
dens-1  — сверхвоздушный UI (минимальная плотность)
dens-3  — стандартная плотность
dens-5  — плотный интерфейс админки
```
Это позволяет менять весь характер UI в один шаг.

## **5. Rarog — масштабируемый как фреймворк, но лёгкий как библиотека**
Он компилируется в **0-runtime CSS**, но при этом сохраняет гибкость уровня Tailwind.
...
Автор: **TheSkiF4er**
— создатель Rarog, движка элементного UI и целой новой школы фронтенд-дизайна.
