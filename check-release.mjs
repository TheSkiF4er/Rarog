...
const checks = [
  ...
  ["package.json", () => includesAll(rootPkg.scripts?.["test:release"] || "", ["test:unit", "test:adapters", "test:contracts"]), "root test:release includes unit, adapter, and contract tests"],
  ...
  [".github/workflows/release.yml", () => releaseWorkflow.includes("npm run test:release"), "release workflow runs the canonical release test gate before publish"],
  ...
  [".github/workflows/docs.yml", () => docsWorkflow.includes("npm run docs:check"), "docs workflow runs docs:check"],
