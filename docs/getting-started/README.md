...
Раздел для первого знакомства с Рарог, быстрого старта и выбора подходящего сценария интеграции.
...
Рарог хорошо сочетается с проектами на React. В репозитории есть стартовый шаблон:

- `examples/starters/vite-react`

### 1. Стартовый проект

Быстрый запуск стартовый шаблон‑а из монорепы Рарог:
...
Открой `http://localhost:5173` — там уже используется ряд классов Рарог.
...
Классы Рарог хорошо ложатся на React‑компоненты:
...
3. Подключаете Рарог CSS/JS как описано выше (через Vite plugin/JIT).
...
### Accessibility in React оболочки
...
В репозитории есть Nuxt/Vue‑стартовый шаблон (`examples/starters/nuxt-rarog`), а также
React/Vite сценарий.
...
### 4. @rarog/vue и Nuxt‑стартовый шаблон
...
### Accessibility in Vue оболочки

Для `@rarog/vue` действуют те же правила, что и для чистого JS core:

- модалки и боковая панель должны содержать реальный title/body для корректного `aria-labelledby` и `aria-describedby`;
...
Рарог можно использовать и в приложениях на Next.js (13/14+).
...
Любой из HTML‑Компоновка’ов из `examples/ui-kits` можно перенести в `app/` или
`pages/`:

- Admin Панель → `/app/(dashboard)/page.tsx`;
- Landing Kit → `/app/page.tsx`;
- SaaS Стартовый шаблон → `/app/(auth)/login/page.tsx`, `/app/(app)/dashboard/page.tsx` и т.п.

Важно:

- классы Рарог остаются без изменений;
- сборка CSS/JS происходит через `rarog build` и подключается в ___KEEP___1___/layout.
...
Этот гайд показывает, как интегрировать Рарог в Laravel‑проект. В репозитории
есть черновой стартовый шаблон:
...
(Если ты используешь монорепу с самим Рарог, можно линковать пакет локально.)

### 2. Конфигурация Рарог
...
Рарог сгенерирует CSS (по умолчанию `dist/rarog.css` или `dist/rarog.jit.css`
в зависимости от режима).

### 4. Подключение в Blade

Подключи CSS в Компоновка, который используется во всех Blade‑шаблонах:
...
В монорепе Рарог есть готовые примеры Компоновка’ов, которые можно напрямую
...
- `examples/ui-kits/saas-starter/*.html` — SaaS‑Компоновка’ы.

Рекомендуемый подход:

1. Скопировать нужный HTML‑Компоновка в `resources/views/...`.
2. Заменить демо‑данные на Blade‑переменные.
3. Убедиться, что Рарог собирается через `rarog build` и подключён в Компоновка’ах.
...
Рарог задуман как «родной» CSS‑слой для экосистемы Cajeer (Cajeer CMS, CajeerEngine,
Farog, Warog и др.), но не привязан к ней жёстко.

Этот гайд описывает общие принципы интеграции.

### 1. Где жить Рарог в Cajeer‑стеке
...
- использовать `rg-container-lg`, `rg-row`, `rg-col-*` для Компоновка;
...
Комбинируя Рарог с Cajeer‑бэком, ты получаешь:
...
Рарог UI‑киты удобно использовать как базовые Компоновка’ы для админок и лендингов
Cajeer:

- Admin Панель — панели управления CajeerEngine/новыми CMS;
- Landing Kit — промо‑страницы продуктов Cajeer;
- SaaS Стартовый шаблон — кабинеты и биллинг‑разделы.

Подход:

- копируете HTML в шаблоны Cajeer;
- оставляете классы Рарог как есть;
- подключаете Рарог как общий CSS/JS‑слой для всего проекта.
