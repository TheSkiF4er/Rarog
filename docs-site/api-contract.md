...
## Контракт событийного lifecycle для overlay/interactive-компонентов

Следующие семейства событий входят в стабильный публичный контракт 3.x:

- `rg:toast:show`, `rg:toast:shown`, `rg:toast:hide`, `rg:toast:hidden`
- `rg:tooltip:show`, `rg:tooltip:shown`, `rg:tooltip:hide`, `rg:tooltip:hidden`
- `rg:popover:show`, `rg:popover:shown`, `rg:popover:hide`, `rg:popover:hidden`
- `rg:collapse:show`, `rg:collapse:shown`, `rg:collapse:hide`, `rg:collapse:hidden`

Гарантии lifecycle:

- `show` и `hide` — pre-event, их можно отменить через `event.preventDefault()`.
- `shown` и `hidden` — post-event, они приходят только после изменения DOM/состояния.
- `event.detail.instance` содержит инстанс компонента.
- `event.detail.trigger` указывает на исходный trigger, если он есть.
- `event.detail.target` указывает на управляемый элемент.
- `Tooltip` и `Popover` дополнительно могут передавать `event.detail.placement`.

Этот контракт намеренно выровнен с уже стабилизированным lifecycle для `modal`, `dropdown`, `offcanvas` и `carousel`, чтобы код приложения мог подписываться на все overlay-компоненты одинаково.
