...
4. **JS Core**
   ...
   - События:
     ...
     - аналогичные lifecycle-события для `dropdown` и `offcanvas`;
       для `carousel` стабильным контрактом считаются `rg:carousel:slide`,
       `rg:carousel:slid` и legacy `rg:carousel:next|prev|goto`.
