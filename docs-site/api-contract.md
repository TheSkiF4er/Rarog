...
## Контракт lifecycle-событий для Toast / Tooltip / Popover / Collapse
...
- `show` и `hide` — pre-event и могут быть отменены через `event.preventDefault()`;
- `shown` и `hidden` — post-event и приходят после изменения DOM/состояния;
- `event.detail.instance` содержит инстанс компонента;
- `event.detail.trigger` указывает на исходный trigger, если он применим;
- `event.detail.target` указывает на управляемый DOM-элемент;
- `Tooltip` / `Popover` могут передавать `event.detail.placement`.

Этот контракт намеренно выровнен с уже стабилизированным lifecycle для `modal`, `dropdown`, `offcanvas` и `carousel`.
