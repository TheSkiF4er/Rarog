...
## JS lifecycle contract (stable in 3.x)

Для публичных JS-компонентов используется единый lifecycle-контракт:
...
Для `carousel` используется совместимый переходный контракт:

- `rg:carousel:slide` — до смены слайда, **cancelable**;
- `rg:carousel:slid` — после смены слайда;
- legacy-события `rg:carousel:next|prev|goto` сохраняются после `slid` ради backward compatibility.
