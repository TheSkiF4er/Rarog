...
Спасибо, что хотите внести вклад в Рарог.

Этот документ описывает **текущий фактический порядок работы репозитория**: какие команды запускать, какие проверки обязательны и как не сломать публичный контур.
...
5. Перед PR прогоните:
 ...
 Если изменение затрагивает React/Vue согласующие слои или их `dist`-контур, дополнительно прогоните `npm run test:adapters`.
...
- средство командной строки-команды или их аргументы;
- версии, релизные баннеры, пакетные метаданные;
- CSS entrypoints или имена выходных файлов;
- JS интерфейс или поддерживаемые компоненты;
- список реально поставляемых пакетов.

---

## PR перечень проверки

В каждом PR желательно указать:
- что изменилось;
- почему это изменение нужно;
- как проверить;
- затрагивает ли оно Документация, build, средство командной строки, Исполняемое ядро JS или CSS output.
...
Репозиторий уже проходил через несколько этапов расширения, поэтому особенно важно не оставлять в Документация обещания о том, чего код не поставляет.
...
Если вы меняете version strings, package metadata или выпуск Поверхность, сначала прогоните:
...
Дополнительно запускайте `npm run test:adapters`, если затронуты согласующие слои, согласующий слой build или `dist`-артефакты.
